...
func CreateIssueEnt(data *[]models.Order) (bool, error) {
	...
	for _, v := range *data {
		if len(v.OrderDetail) > 0 {
			...
			orderType := v.OrderDetail[0].OrderPlan.OrderType.Title
			invNo := GenerateInvNo(v)
			strExecuteOrderEnt := fmt.Sprintf("INSERT INTO TXP_ISSTRANSENT(ISSUINGKEY, ETDDTE, FACTORY, AFFCODE, BISHPC, CUSTNAME, COMERCIAL, ZONEID, SHIPTYPE, COMBINV, SHIPDTE, PC, SHIPFROM, ZONECODE, NOTE1, NOTE2, ISSUINGMAX, ISSUINGSTATUS, RECISSTYPE,UPDDTE, SYSDTE, UUID, CREATEDBY, MODIFIEDBY, REFINVOICE)VALUES('%s', to_date('%s', 'YYYY-MM-DD'), '%s', '%s', '%s', '%s', '%s', '%d', '%s', '%s', to_date('%s', 'YYYY-MM-DD'), '%s', '%s', '%s', '%s', '%s', %d, 0, '%s',CURRENT_TIMESTAMP,CURRENT_TIMESTAMP,'%s','%s', '%s', '%s')", invNo, v.EtdDate.Format("2006-01-02"), v.Consignee.Factory.Title, v.Consignee.Affcode.Title, v.Consignee.Customer.Title, v.Consignee.Customer.Description, v.Commercial.Title, v.Bioabt, v.Shipment.Title, orderType, v.EtdDate.Format("2006-01-02"), v.Pc.Title, v.ShipForm, v.ZoneCode, v.LoadingArea, v.Privilege, len(v.OrderDetail), v.Consignee.Factory.CdCode, v.ID, empID, empID, invNo)
			if uuID != "" {
